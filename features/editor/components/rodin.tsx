...
import type { FormValues } from "@/features/editor/schema/form-schema"
import { submitRodinJob, checkJobStatus, downloadModel } from "@/features/editor/services/api-service"
...
export default function Rodin() {
          ...
          if (downloadData.list && downloadData.list.length > 0) {
            ...
            if (glbFile) {
              ...
              const newModel: GeneratedModel = {
                ...
                prompt: result?.prompt || "Generated Model",
                timestamp: Date.now(),
              }
  ...
  const handleSelectModel = (model: GeneratedModel) => {
  ...
  const handleDeleteModel = (id: string) => {
    setGeneratedModels((prev) => prev.filter((model) => model.id !== id))
  }
  ...
  return (
    <div className="relative h-[100dvh] w-full">
      ...
      {!showPromptContainer && generatedModels.length > 0 && (
        <ModelsGrid models={generatedModels} onSelectModel={handleSelectModel} onDeleteModel={handleDeleteModel} />
      )}
